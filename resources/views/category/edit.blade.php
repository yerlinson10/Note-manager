...
<head>
    ...
    <title>{{ env('APP_NAME') }} - Editar categoría</title>
...
<body class="layout-boxed">
    ...
    <main class="main-container" id="container">
        ...
        <!--  BEGIN SIDEBAR  -->
        <x-sidebar ruta="category"></x-sidebar>
        <!--  END SIDEBAR  -->
                        ...
                        <form action="{{route('categories.update', $category->id)}}" method="POST">
                            ...
                            <div class="row mb-4">
                                <div class="col-sm-12">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Nombre de la categorías *" value="{{old('name', $category->name)}}">
                                </div>
                            </div>
                            <div class="row mb-4">
                                <div class="col-sm-12">
                            ...
                            <div class="row mb-4">
                                <div class="col-sm-12">
                                    ...
                                    <select id="color" name="color" class="form-select" placeholder="Seleccionar un color..." autocomplete="off">
                                        ...
                                        <option value="note-social, Morado" {{ old('color') == sprintf("%s,%s", $category->class, $category->color) || sprintf("%s,%s", $category->class, $category->color)  == 'note-social, Morado' ? 'selected' : ''}}>Morado</option>
                                        <option value="note-personal, Verde" {{ old('color') == sprintf("%s,%s", $category->class, $category->color) || sprintf("%s,%s", $category->class, $category->color)  == 'note-personal, Verde' ? 'selected' : ''}}>Verde</option>
                                        <option value="note-work, Amarillo" {{ old('color') == sprintf("%s,%s", $category->class, $category->color) || sprintf("%s,%s", $category->class, $category->color)  == 'note-work, Amarillo' ? 'selected' : ''}}>Amarillo</option>
                                        <option value="note-important, Rojo" {{ old('color') == sprintf("%s,%s", $category->class, $category->color) || sprintf("%s,%s", $category->class, $category->color)  == 'note-important, Rojo' ? 'selected' : ''}}>Rojo</option>
                                    </select>
                                </div>
                            </div>
